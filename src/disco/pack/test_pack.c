...
make_transaction1_with_jito_tip( fd_txn_p_t * txnp,
  ...
  *p = 's' + 0x80; fd_memcpy( p+1, &i, sizeof(ulong) ); memset( p+9, 'S', FD_TXN_ACCT_ADDR_SZ-9 ); p += FD_TXN_ACCT_ADDR_SZ;
  ...
  fd_memcpy( p, &JITO_MAINNET_TIP_ACCOUNTS[ jito_tip_account ], FD_TXN_ACCT_ADDR_SZ ); p += FD_TXN_ACCT_ADDR_SZ;
  ...
  p[1] = 2;
  p+=2;
  fd_memcpy( p, (uchar[12]){ 0x02, 0x00, 0x00, 0x00, 0xce, 0x56, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }, 12 );
  t->instr[ 2 ].program_id = prog_start+1;
  t->instr[ 2 ].acct_cnt = (ushort)write_accounts;
  t->instr[ 2 ].data_sz = TRANSFER_IX_MIN_SZ;
  t->instr[ 2 ].data_off = (ushort)(p - p_base);
  p += TRANSFER_IX_MIN_SZ;


  /* 4 corresponds to SetComputeUnitPrice */
  ulong rewards_per_cu = (ulong) (pow( 5.0, priority )*10000.0 / (double)compute);
  *p = 4; fd_memcpy( p+1, &rewards_per_cu, sizeof(ulong) );


  
  t->instr[ 3 ].program_id = prog_start;
  t->instr[ 3 ].acct_cnt = 0;
  t->instr[ 3 ].data_sz = 9;
  t->instr[ 3 ].acct_off = (ushort)(p - p_base);
  t->instr[ 3 ].data_off = (ushort)(p - p_base);
  p += 9UL;
...
void test_bundle0( void ) {
  ...
  fd_txn_e_t * _bundle2[3];
  ulong _deleted2;
  fd_txn_e_t * const * bundle2 = fd_pack_insert_bundle_init( pack, _bundle2, 3UL );
  ...
  make_transaction1( bundle2[1]->txnp, 4,  500U, 300U, 12.0, "M",    "N", &reward, &cost_estimate );  total_cost_estimate2 += cost_estimate; total_rewards2 += reward;
  make_transaction1_with_jito_tip( bundle2[2]->txnp, 5,  500U, 300U, 11.0, "M",    "N", &reward, &cost_estimate , 3 );  total_cost_estimate2 += cost_estimate; total_rewards2 += reward;
  
  result |= fd_pack_insert_bundle_fini( pack, bundle2, 3UL, 1000UL, 0, NULL, &_deleted2 );
  ...
  make_transaction1( bundle3[0]->txnp, 6,  500U, 300U, 5.50, "A",    "B", &reward, &cost_estimate );  total_cost_estimate3 += cost_estimate; total_rewards3 += reward;
  make_transaction1( bundle3[1]->txnp, 7,  500U, 300U, 6.5, "C",    "H", &reward, &cost_estimate );  total_cost_estimate3 += cost_estimate; total_rewards3 += reward;
  make_transaction1( bundle3[2]->txnp, 8,  500U, 300U, 4.0, "I",    "J", &reward, &cost_estimate );  total_cost_estimate3 += cost_estimate; total_rewards3 += reward;
  make_transaction1( bundle3[3]->txnp, 9,  500U, 300U, 4.1, "T",    "U", &reward, &cost_estimate );  total_cost_estimate3 += cost_estimate; total_rewards3 += reward;
  ...
  schedule_validate_microblock_bundle( pack, total_cost_estimate2, 0.0f, 3UL, total_rewards2, 0UL, &outcome, 0UL );
  schedule_validate_microblock_bundle( pack, total_cost_estimate, 0.0f, 3UL, total_rewards, 0UL, &outcome, 3UL );
  schedule_validate_microblock_bundle( pack, total_cost_estimate3, 0.0f, 4UL, total_rewards3, 0UL, &outcome, 6UL );

  ulong expected[ 10 ] = { 3,4,5,0,1,2,6,7,8,9 };
  ...
  /*char test_decode[ FD_BASE64_ENC_SZ( FD_TPU_MTU ) ] = "";*/
  /*uchar payload[ FD_TPU_MTU ];*/
  /*fd_base64_decode( payload, test_decode, FD_BASE64_ENC_SZ( FD_TPU_MTU ) );*/
  /*FD_LOG_NOTICE(( "" ));*/
}
