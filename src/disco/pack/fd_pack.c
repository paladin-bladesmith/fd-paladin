    ...
    for( ulong i=0UL; i<txn->instr_cnt; i++ ){
      ...
      if( FD_UNLIKELY( 
        ...
        for( ulong tip_acc_idx=0UL; tip_acc_idx<8UL; tip_acc_idx++ ){
          if( FD_LIKELY( 0==fd_memeq( dest_acct, MAINNET_TIP_ACCOUNTS[ tip_acc_idx ], FD_TXN_ACCT_ADDR_SZ ) ) ) continue;
...
insert_bundle_impl( fd_pack_t           * pack,
  ...
  for( ulong i=0UL; i<txn_cnt; i++ ){
    ...
    cummulative_rew     += (ord->rewards + tips);
    tips_t              += tips;
    ...
    ord->idx = i;
  FD_LOG_NOTICE(( "Bundle %lu has txn %lu r:%i c:%u", bundle_idx, i, ord->rewards, ord->compute_est ));
  }
  ...
  FD_LOG_NOTICE(( "Bundle %lu has tip %lu r:%i c:%u", bundle_idx, tips_t, cummulative_rew, cummulative_compute ));
